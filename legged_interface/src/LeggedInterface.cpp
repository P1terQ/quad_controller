...
void LeggedInterface::setupOptimalControlProblem(const std::string& taskFile, const std::string& urdfFile, const std::string& referenceFile,
  ...
  RelaxedBarrierPenalty::Config barrierPenaltyConfigFootPlcement;
  barrierPenaltyConfigFootPlcement = loadFootPlacementSettings(taskFile, verbose);
  ...
  for (size_t i = 0; i < centroidalModelInfo_.numThreeDofContacts; i++) {
    ...
    problemPtr_->softConstraintPtr->add(footName + "_footPlacement", getFootPlacementConstraint(*eeKinematicsPtr, i, barrierPenaltyConfigFootPlcement));
...
void LeggedInterface::setupModel(const std::string& taskFile, const std::string& urdfFile, const std::string& referenceFile,
  ...
  (
      *pinocchioInterfacePtr_, 
      centroidal_model::loadCentroidalType(taskFile), // dynamic model type
      centroidal_model::loadDefaultJointState(pinocchioInterfacePtr_->getModel().nq - 6, referenceFile),  // default joint state in reference.info
      modelSettings_.contactNames3DoF,  // 4 foot names
...
void LeggedInterface::setupReferenceManager(const std::string& taskFile, const std::string& urdfFile, const std::string& referenceFile,
  ...
  referenceManagerPtr_ =  // initModeSchedule, defaultModeSequenceTemplate. modelSettings_.phaseTransitionStanceTime
...
RelaxedBarrierPenalty::Config LeggedInterface::loadFootPlacementSettings(const std::string& taskFile, bool verbose) const
{
...
std::unique_ptr<StateInputCost> LeggedInterface::getFootPlacementConstraint(const EndEffectorKinematics<scalar_t>& eeKinematics, size_t contactPointIndex, const RelaxedBarrierPenalty::Config& barrierPenaltyConfig) 
{
  std::unique_ptr<FootPlacementConstraint> footPlacementConstraintPtr(new FootPlacementConstraint(*referenceManagerPtr_, eeKinematics, contactPointIndex));
